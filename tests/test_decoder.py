...
def decoder_models() -> tuple[torch.nn.Module, eqx.Module]:
    ...
    return hf_model, eqx_full_model
...
def test_decoder_embeddings(decoder_models):
    ...
    # Token embeddings
    hf_tok = hf_model.decoder.embed_tokens(dummy_input)
    eqx_tok = jax.vmap(eqx_model.decoder.embed_tokens)(
    ...
    hf_pos = hf_model.decoder.embed_positions(positions)
    eqx_pos = eqx_model.decoder.embed_positions.weight[: dummy_input.shape[1]]
...
def test_decoder_self_attn_q_projection(decoder_models, layer_idx: int):
    ...
    hf_proj = hf_model.decoder.layers[layer_idx].self_attn.q_proj
    eqx_proj = eqx_model.decoder.layers[layer_idx].self_attn.q_proj
    diff(
...
def test_decoder_self_attn_k_projection(decoder_models, layer_idx: int):
    hf_model, eqx_model = decoder_models
    dummy_input = torch.randn(5, 384)
    hf_proj = hf_model.decoder.layers[layer_idx].self_attn.k_proj
    eqx_proj = eqx_model.decoder.layers[layer_idx].self_attn.k_proj
    diff(
        eqx_proj(jnp.array(dummy_input.numpy())),
        hf_proj(dummy_input),
...
def test_decoder_self_attn_v_projection(decoder_models, layer_idx: int):
    hf_model, eqx_model = decoder_models
    dummy_input = torch.randn(5, 384)
    hf_proj = hf_model.decoder.layers[layer_idx].self_attn.v_proj
    eqx_proj = eqx_model.decoder.layers[layer_idx].self_attn.v_proj
    diff(
        eqx_proj(jnp.array(dummy_input.numpy())),
        hf_proj(dummy_input),
...
def test_decoder_self_attn_out_projection(decoder_models, layer_idx: int):
    hf_model, eqx_model = decoder_models
    dummy_input = torch.randn(5, 384)
    hf_proj = hf_model.decoder.layers[layer_idx].self_attn.out_proj
    eqx_proj = eqx_model.decoder.layers[layer_idx].self_attn.out_proj
    diff(
        eqx_proj(jnp.array(dummy_input.numpy())),
        hf_proj(dummy_input),
...
def test_decoder_cross_attn_q_projection(decoder_models, layer_idx: int):
    hf_model, eqx_model = decoder_models
    dummy_input = torch.randn(5, 384)
    hf_proj = hf_model.decoder.layers[layer_idx].encoder_attn.q_proj
    eqx_proj = eqx_model.decoder.layers[layer_idx].encoder_attn.q_proj
    diff(
        eqx_proj(jnp.array(dummy_input.numpy())),
        hf_proj(dummy_input),
...
def test_decoder_cross_attn_k_projection(decoder_models, layer_idx: int):
    hf_model, eqx_model = decoder_models
    dummy_input = torch.randn(5, 384)
    hf_proj = hf_model.decoder.layers[layer_idx].encoder_attn.k_proj
    eqx_proj = eqx_model.decoder.layers[layer_idx].encoder_attn.k_proj
    diff(
        eqx_proj(jnp.array(dummy_input.numpy())),
        hf_proj(dummy_input),
...
def test_decoder_cross_attn_v_projection(decoder_models, layer_idx: int):
    hf_model, eqx_model = decoder_models
    dummy_input = torch.randn(5, 384)
    hf_proj = hf_model.decoder.layers[layer_idx].encoder_attn.v_proj
    eqx_proj = eqx_model.decoder.layers[layer_idx].encoder_attn.v_proj
    diff(
        eqx_proj(jnp.array(dummy_input.numpy())),
        hf_proj(dummy_input),
...
def test_decoder_cross_attn_out_projection(decoder_models, layer_idx: int):
    hf_model, eqx_model = decoder_models
    dummy_input = torch.randn(5, 384)
    hf_proj = hf_model.decoder.layers[layer_idx].encoder_attn.out_proj
    eqx_proj = eqx_model.decoder.layers[layer_idx].encoder_attn.out_proj
    diff(
        eqx_proj(jnp.array(dummy_input.numpy())),
        hf_proj(dummy_input),
...
def test_decoder_self_attn_layer_norm(decoder_models, layer_idx: int):
    hf_model, eqx_model = decoder_models
    dummy_input = torch.randn(5, 384)
    hf_ln = hf_model.decoder.layers[layer_idx].self_attn_layer_norm
    eqx_ln = eqx_model.decoder.layers[layer_idx].self_attn_layer_norm
...
def test_decoder_cross_attn_layer_norm(decoder_models, layer_idx: int):
    hf_model, eqx_model = decoder_models
    dummy_input = torch.randn(5, 384)
    hf_ln = hf_model.decoder.layers[layer_idx].encoder_attn_layer_norm
    eqx_ln = eqx_model.decoder.layers[layer_idx].encoder_attn_layer_norm
...
def test_decoder_ffn_fc1(decoder_models, layer_idx: int):
    hf_model, eqx_model = decoder_models
    dummy_input = torch.randn(5, 384)
    hf_fc1 = hf_model.decoder.layers[layer_idx].fc1
    eqx_fc1 = eqx_model.decoder.layers[layer_idx].fc1
    diff(
...
def test_decoder_ffn_fc2(decoder_models, layer_idx: int):
    ...
    hf_fc2 = hf_model.decoder.layers[layer_idx].fc2
    eqx_fc2 = eqx_model.decoder.layers[layer_idx].fc2
...
def test_decoder_self_attn_full(decoder_models, layer_idx: int):
    ...
    hf_attn = hf_model.decoder.layers[layer_idx].self_attn
    eqx_attn = eqx_model.decoder.layers[layer_idx].self_attn
...
def test_decoder_cross_attn_full(decoder_models, layer_idx: int):
    ...
    hf_attn = hf_model.decoder.layers[layer_idx].encoder_attn
    eqx_attn = eqx_model.decoder.layers[layer_idx].encoder_attn
...
def test_consecutive_decoder_layers(decoder_models, layer_pair: tuple[int, int]):
    ...
    hf_layer1 = hf_model.decoder.layers[layer_idx1]
    hf_layer2 = hf_model.decoder.layers[layer_idx2]
    ...
    eqx_layer1 = eqx_model.decoder.layers[layer_idx1]
    eqx_layer2 = eqx_model.decoder.layers[layer_idx2]
...
def test_full_decoder(decoder_models):
    ...
    with torch.no_grad():
        hf_out = hf_model.decoder(
            input_ids, encoder_hidden_states=dummy_encoder
        ).last_hidden_state

    seq_len = input_ids.shape[-1]
    eqx_out = jax.vmap(eqx_model.decoder)(
